...
- The form theme is global, so it applies to both the Flexy front-office and the back-office. Every widget block that Flexy overrides (password, textarea, money, percent, choice, radio, range, file, submit) needs a mirror block in the back-office form theme that routes `/admin` to `bootstrap_5_layout.html.twig`. Without it, Flexy's wrappers break Bootstrap input groups in the admin.
