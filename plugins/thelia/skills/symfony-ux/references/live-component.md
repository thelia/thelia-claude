...
TwigComponents that re-render dynamically via AJAX. Build reactive UIs in PHP and Twig with zero JavaScript. Every user interaction triggers a server round-trip that re-renders the component and morphs the DOM.
...
- Quick Reference
- Basic Example
- LiveProp (writable, URL binding, hydration)
- LiveProp Route Parameter Binding (2.25+)
- Data Binding (data-model) and Validation Modifiers (2.28+)
- LiveAction
- Search Example
- Loading States
- Form Integration and Real-time Validation
- Multi-Step Form Pattern (Symfony 7.4 Form Flows + LiveComponent)
- Component Communication (emit/listen, browser events)
- Polling
...
Sync a prop to a URL query parameter to enable bookmarkable/shareable state:
...
### Route Parameter Binding (LiveComponent 2.25+)

Map a LiveProp directly to a route segment. The initial value comes from the URL; client-side changes update the URL via the history API and can be bookmarked.
...
final class ProductDetail
{
    ...
    // Bound to /products/{slug} in the route
    #[LiveProp(fromUrl: true)]
    public string $slug = '';

    #[LiveProp(writable: true, url: new UrlMapping(as: 'variant'))]
    public ?string $selectedVariant = null;
}
```

Differences from plain `url: true`:
- `fromUrl: true` reads a route segment, not a query parameter
- The URL stays clean (`/products/foo?variant=xl` instead of `/products?slug=foo&variant=xl`)
- Compatible with `#[Route('/products/{slug}')]`
...
### Validation Modifiers (since 2.28)
...
Symfony 7.4 introduced Form Flows (native multi-step forms). Combined with LiveComponent, you get a reactive multi-step form with per-step validation and no full reload.

### Variant 1: Symfony 7.4 Form Flow inside a LiveComponent
...
#[AsLiveComponent]
final class RegistrationFlowComponent extends AbstractController
{
    use DefaultActionTrait;
    use ComponentWithFormTrait;
    ...
    #[LiveProp]
    public ?RegistrationData $data = null;

    public function __construct(
        private readonly FormFlowInterface $flow,
    ) {}

    protected function instantiateForm(): FormInterface
    {
        return $this->flow->getFormForStep($this->currentStep, $this->data);
    }
    ...
    public function nextStep(): void
    {
        ...
        if (!$this->getForm()->isValid()) {
            return; // Re-render with errors, same step
        }
    ...
    #[LiveAction]
    public function submit(UserRegistrar $registrar): Response
    {
        $this->submitForm();
        if (!$this->getForm()->isValid()) {
            return $this->redirectToRoute('registration_start');
        }
        $registrar->register($this->data);
        return $this->redirectToRoute('registration_success');
    }
}
```

### Variant 2: Multi-step without Form Flow (simple cases)

For a 2-3 step flow without complex shared state, a standalone LiveComponent is enough:
...
final class ProjectWizard
{
    ...
    {
        match ($this->step) {
            1 => $this->projectName !== '' ?: throw new \DomainException('Name required'),
            2 => $this->projectType !== '' ?: throw new \DomainException('Type required'),
            default => null,
        };
    }
}
...
#[Assert\Choice(choices: ['web', 'mobile'], groups: ['step2'])]
public string $projectType;
```

In `instantiateForm()` or at validation time:

```php
$this->validator->validate($this->data, groups: ["step{$this->currentStep}"]);
```
...
<div {{ attributes }}>
    ...
    {% elseif currentStep == 3 %}
        {{ form_row(form.budgetInCents) }}
    {% endif %}

    <div class="actions">
        {% if currentStep > 1 %}
            <button data-action="live#action" data-live-action-param="previousStep">Previous</button>
        {% endif %}
        {% if currentStep < totalSteps %}
            <button data-action="live#action" data-live-action-param="nextStep" data-loading="attr(disabled)">Next</button>
        {% else %}
            <button data-action="live#action" data-live-action-param="submit" data-loading="attr(disabled)">Submit</button>
        {% endif %}
    </div>
</div>
```
...
**Use `norender` and `on(blur)` to reduce requests.** Not every keystroke needs a server call. Debounce text inputs and defer binding to blur events for fields that don't need instant feedback.

**`{{ attributes }}` on the root element is non-negotiable.** Without it, the live behavior Stimulus controller is never attached and nothing works.
