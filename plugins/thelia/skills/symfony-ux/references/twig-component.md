...
Reusable UI components with PHP classes and Twig templates. Think React/Vue components, but server-rendered with zero JavaScript.
...
- Props (public properties, mount, dynamic)
- Blocks (Slots)
- Computed Properties
- Attributes
- Components as Services
- Lifecycle Hooks (PreMount, PostMount)
- Nested Components
- Configuration
- HTML vs Twig Syntax
...
Extra HTML attributes passed to the component are available via `{{ attributes }}`. This is how you let consumers add custom classes, ids, data attributes, and so on.
...
Components are Symfony services, so autowiring works naturally. Use the constructor for dependencies and public properties for props.
...
Use HTML syntax (`<twig:...>`) in all cases. The Twig syntax (`{% component %}`) is legacy and should not appear in new code.
