...
| Rename a Smarty loop | snake_case auto (`BetterSeoLoop` -> `better_seo_loop`); alias via `<loop name="alias">` | hooks-loops.md |
...
| 7 | `postDeactivation` | Inside the transaction | (none) |
| 8 | `destroy` | Manual removal (`$deleteModuleData=true` purges tables) | (none) |
...
| 2 | **`new XxxRepository()` inline** bypasses DI, crashes if repository acquires a dependency | review acceptance | `$this->getContainer()->get(MyRepository::class)` in inherited methods without injection (`pay`, `getPostage`...) |
| 3 | **BO configuration without dedicated Controller**: save returns 404 or overwrites core config | `ModuleController::processUpdateAction` core CRUD | `routing.xml` + `Controller/Admin/` + `{token_url}` in template (see CustomDelivery) |
...
- [references/migration.md](references/migration.md): T2.5 -> T2.6 (AP 3.4, SF 6.4, Propel filters); T2 -> T3 migration is out of scope
