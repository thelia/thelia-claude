...
- Filters: `#[ApiFilter]` with **Thelia custom Propel filters only** (no Doctrine -- runtime crash)
...
| `#[AsEventListener]` SF 6.x | Yes -- replace XML |
| `#[AsCommand]` SF 6.4 | Yes -- already standard |
| `#[AutowireIterator]` / `#[AutowireLocator]` | Yes -- prepare for SF 7.1+ |
| `final readonly class` PHP 8.2 | Yes, required for stateless services and DTOs |
| `enum` PHP 8.1 | Yes -- replace string constants |
| `match` expressions | Yes -- replace `switch` |
...
| `IntegrationTestCase` / `FixtureFactory` / `ApiTestCase` | Do not exist in T2 (vs T3) -- custom harness |
